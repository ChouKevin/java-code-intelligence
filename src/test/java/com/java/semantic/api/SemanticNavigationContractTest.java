...
class SemanticNavigationContractTest {
    ...
    void should_compose_status_revision_and_resolved_target_into_exact_method_source_request() throws Exception {
        ...
        mockMvc.perform(post("/v1/discovery/method-source")
                ...
                .andExpect(jsonPath("$.availableFollowUps[3].operation").value("GET_TYPE_MEMBERS"))
                ...
                .andExpect(jsonPath("$.availableFollowUps[3].request.repoId").value(REPOSITORY_ID.value()))
                .andExpect(jsonPath("$.availableFollowUps[3].request.expectedRevision").value(currentRevision))
                .andExpect(jsonPath("$.availableFollowUps[3].request.sourceType.sourceFile")
                        .value(SOURCE_TYPE.sourceFile()))
                .andExpect(jsonPath("$.availableFollowUps[3].request.sourceType.javaType.packageName")
                        .value(SOURCE_TYPE.javaType().packageName()))
                .andExpect(jsonPath("$.availableFollowUps[3].request.sourceType.javaType.className")
                        .value(SOURCE_TYPE.javaType().className()))
                .andExpect(jsonPath("$.availableFollowUps[3].request.memberKinds[0]").value("FIELD"))
                .andExpect(jsonPath("$.availableFollowUps[3].request.offset").value(0))
                .andExpect(jsonPath("$.availableFollowUps[3].request.limit").value(50));
    }
    ...
    void should_append_exact_java_source_continuation_after_complete_method_navigation() throws Exception {
        ...
        mockMvc.perform(post("/v1/discovery/method-source")
                        ...
                        .header(ApiTokenFilter.API_TOKEN_HEADER, TOKEN))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.availableFollowUps.length()").value(5))
                .andExpect(jsonPath("$.availableFollowUps[0].operation").value("GET_METHOD_SOURCE"))
                .andExpect(jsonPath("$.availableFollowUps[1].operation")
                        .value("ANALYZE_OUTGOING_CALL_GRAPH"))
                .andExpect(jsonPath("$.availableFollowUps[2].operation")
                ...
                .andExpect(jsonPath("$.availableFollowUps[3].request.repoId").value(REPOSITORY_ID.value()))
                .andExpect(jsonPath("$.availableFollowUps[3].request.expectedRevision").value(REVISION.value()))
                .andExpect(jsonPath("$.availableFollowUps[3].request.sourceType.sourceFile")
                        .value(SOURCE_TYPE.sourceFile()))
                .andExpect(jsonPath("$.availableFollowUps[3].request.sourceType.javaType.packageName")
                        .value(SOURCE_TYPE.javaType().packageName()))
                ...
                .andExpect(jsonPath("$.availableFollowUps[3].request.offset").value(0))
                .andExpect(jsonPath("$.availableFollowUps[3].request.limit").value(50))
                .andExpect(jsonPath("$.availableFollowUps[4].operation")
                        .value("GET_SOURCE_SEGMENT"))
                .andExpect(jsonPath("$.availableFollowUps[4].api.method").value("POST"))
                .andExpect(jsonPath("$.availableFollowUps[4].api.path")
                        .value("/v1/discovery/source-segment"))
                .andExpect(jsonPath("$.availableFollowUps[4].request.repoId")
                        .value(REPOSITORY_ID.value()))
                .andExpect(jsonPath("$.availableFollowUps[4].request.expectedRevision")
                        .value(REVISION.value()))
                .andExpect(jsonPath("$.availableFollowUps[4].request.location.sourceFile")
                        .value(SOURCE_TYPE.sourceFile()))
                .andExpect(jsonPath("$.availableFollowUps[4].request.location.range.start.line").value(4))
                .andExpect(jsonPath("$.availableFollowUps[4].request.location.range.start.character").value(0))
                .andExpect(jsonPath("$.availableFollowUps[4].request.location.range.end.line").value(8))
                .andExpect(jsonPath("$.availableFollowUps[4].request.location.range.end.character").value(1))
                .andExpect(jsonPath("$.availableFollowUps[4].request.contextLines").value(0));
    }
