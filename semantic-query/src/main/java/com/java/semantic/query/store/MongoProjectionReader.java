...
abstract class MongoProjectionReader {
    ...
    final List<Document> selected(String collection, RepositoryId repositoryId) {
        try {
            MongoCurrentGenerationReader.CurrentGeneration generation = current.read(repositoryId);
