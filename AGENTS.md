...
`java-semantic-service` is a standalone Java 21/Spring Boot Maven project; it is not a module of the Java System Agent. This repository, `git@github.com:ChouKevin/java-code-intelligence.git`, is its canonical and sole source. R0 history-preserving extraction is complete and R1 independent delivery is active. R1 adds independent CI, a reproducible Docker build, and a pinned JDT LS installation only; image publication and deployment remain out of scope. Never recreate or maintain a parallel service implementation in the Agent repository. Production code lives under `src/main/java/com/java/semantic`. Packages separate HTTP contracts (`api`), graph construction (`callgraph`), repository lifecycle (`repository`), JDT LS integration (`semantic`), JDT syntax extraction (`syntax`), route matching (`trie`), and shared identity/diagnostics/configuration. Runtime configuration is in `src/main/resources/application.yml`; the versioned contract is `src/main/resources/openapi/semantic-api-v1.yaml`. Tests mirror production packages under `src/test/java`, with fixture repositories in `src/test/resources/fixtures`.
